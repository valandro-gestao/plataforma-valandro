...
### 17.1 Contratos de dados entre produtor e consumidor

- **[Obrigatório]** Quando uma aplicação, processo ou serviço (produtor) gera dados consumidos por outro sistema (consumidor) — configuração, classificação, arquivo de integração, resposta de API, etc. — vale o princípio `Produtor → Contrato de dados → Consumidor`: o produtor não pode alterar unilateralmente estrutura, campos, hierarquia ou semântica do seu output enquanto houver consumidor dependente daquele contrato.
- **[Obrigatório]** Mudanças no contrato de dados devem ser identificadas explicitamente, avaliadas quanto ao impacto no(s) consumidor(es) existente(s), compatibilizadas com eles antes de entrarem em produção, e documentadas antes de serem consideradas padrão.
- **[Recomendado]** Ver `docs/PADRAO_PROCESSOS_ASSISTIDOS_IA.md` (seção 7) para o caso real que originou este princípio — um processo assistido por IA cuja evolução de interpretação de dados passou a produzir uma estrutura mais rica do que o motor consumidor suportava na época.
...
| 17 | Contratos de dados entre produtor e consumidor — mudanças de estrutura, campos ou semântica de um output consumido por outro sistema exigem identificação, avaliação de impacto, compatibilização com os consumidores e documentação antes de virarem padrão | Arquitetura | Obrigatório |
...
| 1.2 | 20/08/2026 | Adicionada a seção 17.1 — Contratos de dados entre produtor e consumidor, formalizando o princípio de compatibilidade entre produtor e consumidor de dados a partir do caso real documentado em PADRAO_PROCESSOS_ASSISTIDOS_IA.md; registrada decisão 17 | **Aprovado — oficial** |
